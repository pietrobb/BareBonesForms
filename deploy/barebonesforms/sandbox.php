...
if (!is_string($action) || !isset($operations[$action])) {
    ...
    bbf_audit_write($config, $principal, 'sandbox_invalid', '', [], 'denied', 'attempted', 0);
    bbf_audit_write($config, $principal, 'sandbox_invalid', '', [], 'denied', 'failed', 0);
...
function bbf_sandbox_definition(string $file): array {
    if (!is_file($file) || is_link($file)) throw new RuntimeException('Invalid definition file');
    $json = @file_get_contents($file);
    if ($json === false) throw new RuntimeException('Definition read failed');
    $def = json_decode($json, false, 512, JSON_THROW_ON_ERROR);
    if (!$def instanceof stdClass || (isset($def->name) && !is_string($def->name))
        || (property_exists($def, 'fields') && !is_array($def->fields))) {
        throw new RuntimeException('Invalid definition');
    }
    foreach ($def->fields ?? [] as $field) {
        if (!$field instanceof stdClass) throw new RuntimeException('Invalid field');
    }
    return [$json, $def];
}
function bbf_sandbox_error(int $code, string $message): void {
    bbf_access_finish(0, false);
...
try {
    // Definition requests must not enumerate or depend on unrelated forms.
    if ($action === 'definition') {
        if ($fid === '' || !bbf_auth_definition_identity($config, $fid)) bbf_sandbox_error(400, 'Invalid form');
        $file = $formsDir . "/$fid.json";
        if (!file_exists($file)) bbf_sandbox_error(404, 'Form not found');
        [$json, $def] = bbf_sandbox_definition($file);
        bbf_access_finish(1); // One definition actually read and validated.
        header('Content-Type: application/json');
        echo $json;
        exit;
    }
    if (!is_dir($formsDir) || is_link($formsDir)) throw new RuntimeException('Invalid forms directory');
    $formFiles = glob($formsDir . '/*.json');
    if ($formFiles === false) throw new RuntimeException('Forms enumeration failed');
    $forms = [];
    foreach ($formFiles as $f) {
        $basename = basename($f, '.json');
        if ($basename === 'form.schema') continue;
        if (!bbf_auth_definition_identity($config, $basename)) throw new RuntimeException('Invalid definition identity'); [, $def] = bbf_sandbox_definition($f);
        $forms[] = ['id' => $basename, 'name' => $def->name ?? $basename, 'fields' => count($def->fields ?? [])];
    }
    if ($action === 'forms') {
        $json = json_encode($forms, JSON_THROW_ON_ERROR);
        bbf_access_finish(count($forms)); // Actual forms returned, excluding the schema.
        header('Content-Type: application/json');
        echo $json;
        exit;
    }
} catch (Throwable $error) {
    bbf_sandbox_error(500, 'Unable to load sandbox forms');
}

$selectedForm = bbf_auth_id($_GET['form'] ?? '') ?: ($forms[0]['id'] ?? '');
// Render completely before recording completion and releasing the page.
ob_start();
?>
<!DOCTYPE html>
<html lang="en">
    ...
    async function sandboxSubmit(formEl, formId) {
        ...
        try {
            const resp = await fetch(`submit.php?form=${encodeURIComponent(formId)}&sandbox=1`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-BBF-CSRF': sandboxCsrf },
...
<?php
$page = ob_get_clean();
bbf_access_finish(count($forms)); // Number of form choices rendered, not a premature zero.
echo $page;
