...
$principal = bbf_authenticate($config, true, true);
bbf_access_begin($config, $principal, 'diagnostics', '', [], true);
// Release the management session before fixed-target HTTP probes.
...
// ─── Authenticated config snapshot ───────────────────────────
// The loader invalidates cached config before authentication.
// Do not reload after authorization: probes and audit must use
// the same configuration snapshot that granted this request.
// Reaching this point means the configuration exists and is valid.
$hasConfig = true;
...
// Never derive a diagnostic destination from Host, SERVER_NAME or REQUEST_URI.
...
bbf_access_finish(count($results));
?><!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
