...
// ─── CSRF validation ────────────────────────────────────────────
// Skip CSRF for valid smoke_token requests (used by smoketest.php)
$_smokeToken = $_SERVER['HTTP_X_BBF_SMOKE_TOKEN'] ?? $_GET['smoke_token'] ?? '';
$_smokeAuth  = !empty($config['smoke_token']) && !empty($_smokeToken)
               && hash_equals($config['smoke_token'], $_smokeToken);
$isCorsRequest = !empty($origin) && !empty($config['allowed_origins'])
    && in_array($origin, $config['allowed_origins'], true);
if (($config['csrf'] ?? true) && !$isCorsRequest && !$_smokeAuth) {
    ensureSession();
...
// ─── Sandbox mode ───────────────────────────────────────────────
// Sandbox is enabled via config, OR via valid smoke_token (for production smoke tests)
$isSandbox = isset($_GET['sandbox']) && (($config['sandbox'] ?? false) || $_smokeAuth);
if ($isSandbox) {
